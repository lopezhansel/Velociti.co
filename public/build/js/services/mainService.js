...
app.service('mainService', ['$routeParams', '$mdMedia', '$mdDialog', '$mdToast', "$http", "$interval", 'leafletData', "$location", function ($routeParams, $mdMedia, $mdDialog, $mdToast, $http, $interval, leafletData, $location) {
	...
	serv.greatCircleMethod = function (destLat, destLng, clientLat, clientLng) {
		...
		var arc = Math.sin(Δφ / 2) * Math.sin(Δφ / 2) + Math.cos(φ1) * Math.cos(φ2) * Math.sin(Δλ / 2) * Math.sin(Δλ / 2);
	...
	// I need This in case user denies permission to share location // hansel
	...
	//  incoming request
	serv.socket.on('allRequests', function (data) {
		// bug: when you log in data is an empty {}
